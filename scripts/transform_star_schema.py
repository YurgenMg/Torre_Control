...
def get_db_connection():
    ...
    try:
        ...
        raise DatabaseConnectionError(
            f"Cannot connect to PostgreSQL: {e}") from e
...
def execute_sql_file(conn, sql_file_path):
    ...
    try:
        ...
        raise SQLExecutionError(
            f"PostgreSQL error: {e.pgcode} - {e.pgerror}") from e
        ...
        with conn.cursor() as cursor:
            ...
            if fact_count == 0:
                ...
                raise DataValidationError(
                    "fact_orders is empty - ETL transformation failed")
...
def main():
    ...
    try:
        ...
        sql_file = Path(__file__).parent.parent / 'sql' / \
            'populate_star_schema_simple.sql'
